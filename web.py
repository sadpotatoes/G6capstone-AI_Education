...
def label():
    ...
    if current_user.is_authenticated:
        user = User.query.filter_by(username = current_user.username).first()
        if Confidence.query.filter_by(user_id = user.id).first():
            """In the case that the user goes home without closing the page and attempts to reload their ML model we'll need to wipe the session of labels and reset the form 
               since they will continue to hold information from the previous run and suplicate them upon loading which will change the confidence rate"""
            form = LabelForm()
            session['labels'] = []
